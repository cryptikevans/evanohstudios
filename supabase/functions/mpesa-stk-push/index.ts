...
const phone = (x: string) => {
  ...
  if (/^0[17]\d{8}$/.test(x)) {
    return '254' + x.slice(1);
  }

  if (/^254[17]\d{8}$/.test(x)) {
    return x;
  }
  ...
  try {
    ...
    let eventResult =
      await db
        .from('events')
        .select(
          'id,name,ticket_price,published,archived'
        )
        .eq(
          'id',
          event_id
    ...
    /*
      ...
      Keep the actual value in Supabase secrets.
    */
    ...
    let pass =
      btoa(
        `${shortcode}${Deno.env.get('MPESA_PASSKEY')}${t}`
      );
    ...
    let update =
      await db
        ...
        .update({
          merchant_request_id:
            sj.MerchantRequestID,

          checkout_request_id:
            sj.CheckoutRequestID
        })
        .eq(
          'id',
          ins.data.id
